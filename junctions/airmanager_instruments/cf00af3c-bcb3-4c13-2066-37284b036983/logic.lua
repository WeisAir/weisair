...
local announciator_dataref_name = user_prop_add_string("Simulator Source Data Name", "laminar/B738/annunciator/at_disconnect1", "The simulator data field to read annunciator status from.")
...
-- check 3rd state of this annunciator off/yellow/red


if user_prop_get(add_lable) == true then
	if user_prop_get(lable_pos) == "top" then 
		annunciator_inactive = img_add("ann_inactive.png",0,20,45,45)
		annunciator_active = img_add("ann_active_yellow.png",0,20,45,45)
		txt_add(user_prop_get(lable_text), lable_txt_style, 0,0,45,20)
	end
	
	if user_prop_get(lable_pos) == "bottom" then 
		annunciator_inactive = img_add("ann_inactive.png",0,0,45,45)
		annunciator_active = img_add("ann_active_yellow.png",0,0,45,45)
		txt_add(user_prop_get(lable_text), lable_txt_style, 0,45,45,20)
	end
	
	if user_prop_get(lable_pos) == "left" then 
		annunciator_inactive = img_add("ann_inactive.png",45,0,45,45)
		annunciator_active = img_add("ann_active_yellow.png",45,0,45,45)
	...
	if user_prop_get(lable_pos) == "right" then 
		annunciator_inactive = img_add("ann_inactive.png",0,0,45,45)
		annunciator_active = img_add("ann_active_yellow.png",0,0,45,45)
		txt_add(user_prop_get(lable_text), lable_txt_style, 45,0,45,45)
	end
	
else 
	if user_prop_get(add_frame) == true then
	    annunciator_inactive = img_add("ann_inactive.png",3,3,82,82) 
	    annunciator_active = img_add("ann_active_yellow.png",3,3,82,82) 
	else
	    annunciator_inactive = img_add_fullscreen("ann_inactive.png") 
	    annunciator_active = img_add_fullscreen("ann_active_yellow.png")
	end 
end
...
function press_button()                                    
    xpl_command ( "WeisAIR/b733/mcp/at_prst_push") 
end
function release_button()                                 
    xpl_command ( "WeisAIR/b733/mcp/at_prst_release") 
end  
push_button = button_add ( nil  ,nil , 3,3,82,82  , press_button, release_button )  
