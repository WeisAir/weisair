...
local xpdr_dot_pos_x_ten = 150
local xpdr_dot_pos_x_thou = 115
...
--xpdr active dot
xpdr_active_digit_dot_ten = img_add("xpdr_active_digit_dot.png",xpdr_dot_pos_x_ten,xpdr_dot_pos_y,3.3*xpdr_knob_diameter,0.5*xpdr_knob_diameter)
xpdr_active_digit_dot_thou = img_add("xpdr_active_digit_dot.png",xpdr_dot_pos_x_thou,xpdr_dot_pos_y,3.3*xpdr_knob_diameter,0.5*xpdr_knob_diameter)
visible(xpdr_active_digit_dot_ten,false)
visible(xpdr_active_digit_dot_thou,false)
...
function callback_outer_knob_turn(direction)
        ...
        visible(xpdr_active_digit_dot_ten,false)
        visible(xpdr_active_digit_dot_thou,true)

	if direction == 1 then  xpl_command("les/sf34a/acft/avio/mnp/pl2_atc_thou_hund_knob_up") end
	if direction == -1 then xpl_command("les/sf34a/acft/avio/mnp/pl2_atc_thou_hund_knob_dn") end
end

function callback_inner_knob_turn(direction)
	
	visible(xpdr_active_digit_dot_ten,true)
        visible(xpdr_active_digit_dot_thou,false)
	
	if direction == 1 then  xpl_command("les/sf34a/acft/avio/mnp/pl2_atc_ten_one_knob_up") end
	if direction == -1 then xpl_command("les/sf34a/acft/avio/mnp/pl2_atc_ten_one_knob_dn") end
end

function callback_knob_press()
	--XPDR ident
	xpl_command("les/sf34a/acft/avio/mnp/pl2_atc_ident_button")
...
function xpdr_values (xpdr_code, xpdr_mode,xpdr_ident)
	...
	if xpdr_mode == 1.0 then txt_set(txt_xpdr_mode, "OFF") end
	if xpdr_mode == 1.0 then txt_set(txt_xpdr_mode, "STBY") end
	if xpdr_mode == 2.0 then txt_set(txt_xpdr_mode, "ON") end
	if xpdr_mode == 3.0 then txt_set(txt_xpdr_mode, "ALT") end
