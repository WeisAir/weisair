...
function annun_value (condition)
    ...
    xpl_dataref_write("XCrafts/SA/SA_toggle", "INT", annun_value_last)
end

function  press_callback()
    if annun_value_last == 0 then
        xpl_dataref_write("XCrafts/SA/SA_toggle", "INT", 1)
        annun_value_last = 1
    else
        xpl_dataref_write("XCrafts/SA/SA_toggle", "INT", 0)
        annun_value_last = 0
    end
end
...
xpl_dataref_subscribe("XCrafts/SA/SA_toggle", "INT", annun_value)