...
function FW_value (condition)
    visible( annunciator_active, condition > 0)
end

function  press_callback()
    xpl_command( "WeisAIR/b733/mcp/fire_warn_push")
end

function release_callback()
    xpl_command( "WeisAIR/b733/mcp/fire_warn_release")
end

btn = button_add(nil,nil,0,0,136,65, press_callback, release_callback)

-- Simulator data subscription
xpl_dataref_subscribe("ixeg/733/firewarning/fire_warning_ann", "FLOAT", FW_value)