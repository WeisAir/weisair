...
img_airplane = img_add("airplane.png", 139, 264, 272, 216, "move_animation_type: LOG; move_animation_speed: 0.05")
...
xpl_dataref_subscribe("sim/cockpit2/gauges/indicators/suction_1_ratio", "FLOAT", 
    ...
    move(img_airplane, nil, 264 - (hoop_height * 1.2), nil, nil)
    
    -- Slip indicator
    local slip_deg = var_cap(slip * 0.9, -6.5, 6.5)
    local x, y = geo_rotate_coordinates(slip_deg + 180, 700)
...
msfs_variable_subscribe("SUCTION PRESSURE", "inHg",
                        ...
                        "ATTITUDE BARS POSITION", "Percent", function(suction, roll, pitch, slip, hoop_height)
    ...
    gbl_hoop_heigt = hoop_height
    -- print(hoop_height)
    move(img_airplane, nil, 264 - var_cap(hoop_height, -30, 30), nil, nil)
    
    -- Slip indicator
    local slip_deg = var_cap(slip * -10, -6.5, 6.5)
    local x, y = geo_rotate_coordinates(slip_deg + 180, 700)
    move(img_ball, x + 257, y + -318, nil, nil)
    rotate(img_ball, slip_deg)
...
function cage_pressed_callback()
    ...
    msfs_event("ATTITUDE_CAGE_BUTTON")
end
