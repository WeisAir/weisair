...
local announciator_dataref_name = user_prop_add_string("Simulator Source Data Name", "laminar/B738/annunciator/emer_exit", "The simulator data field to read annunciator status from.")
...
if user_prop_get(add_lable) == true then
	if user_prop_get(lable_pos) == "top" then 
		annunciator_inactive = img_add("ann_inactive.png",0,20,45,91)
		annunciator_active = img_add("ann_active.png",0,20,45,91)
		txt_add(user_prop_get(lable_text), lable_txt_style, 0,0,45,20)
	end
	
	if user_prop_get(lable_pos) == "bottom" then 
		annunciator_inactive = img_add("ann_inactive.png",0,0,45,91)
		annunciator_active = img_add("ann_active.png",0,0,45,91)
		txt_add(user_prop_get(lable_text), lable_txt_style, 0,91,45,20)
	end
	
	if user_prop_get(lable_pos) == "left" then 
		annunciator_inactive = img_add("ann_inactive.png",45,0,45,91)
		annunciator_active = img_add("ann_active.png",45,0,45,91)
	...
	if user_prop_get(add_frame) == true then
	    annunciator_inactive = img_add("ann_inactive.png",3,3,59,129) 
	    annunciator_active = img_add("ann_active.png",3,3,59,129) 
	else
