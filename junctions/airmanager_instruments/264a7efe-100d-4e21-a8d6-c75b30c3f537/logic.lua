...
function ann_value (condition)
    visible( annunciator_active, condition > 0.4)
end

function  press_callback()
    xpl_command( "les/sf34a/acft/wrng/mnp/master_caution_button_pilot")
end

function release_callback()
    xpl_command( "les/sf34a/acft/wrng/mnp/master_caution_button_pilot",0)
end

btn = button_add(nil,nil,0,0,136,65, press_callback)

-- Simulator data subscription
xpl_dataref_subscribe("LES/saab/annun/master_caution", "FLOAT", ann_value)