...
function PT_cht(cht)
    cht = var_cap(cht[2], 50, 250)
    rotate(img_neddle_cht, -135 + (90 / 200 * (cht - 50)) )
end

function PT_egt(egtraw)
    egt = var_cap(egtraw[2], max_egt - 240, max_egt + 40)
...
fsx_variable_subscribe("RECIP ENG CYLINDER HEAD TEMPERATURE:2", "Celsius", PT_cht_FSX)
fsx_variable_subscribe("GENERAL ENG EXHAUST GAS TEMPERATURE:2", "Celsius", PT_egt_FSX)
msfs_variable_subscribe("RECIP ENG CYLINDER HEAD TEMPERATURE:2", "Celsius", PT_cht_FSX)
msfs_variable_subscribe("GENERAL ENG EXHAUST GAS TEMPERATURE:2", "Celsius", PT_egt_FSX)