...
-- Values to be updated by Authors
 ...
 local Z2DXPMCP = "1.1.6"				-- Current MCP release
 local Date_Release = "06/03/2020"		-- Last MCP release date
...
--	enjxp_SimPassion			02/nn/2019		1.1.n		TODO : green button light min Voltage to switch OFF from Standby Power
...
---------------------------------------------------------------------------------------------------------------------------------------------
--  New release based on initial graphic design by AudioBirdXP and ZiboMod 3.31d , script reworking made by enjxp_SimPassion on 11/02/2018
...
function set_power_xpl(bus_volts,avionics_on,vvi_dial_show,spd_leds_show)	-- Power Handling Based on sample instrument "Boeing 737NG - Comm Nav Stack" from Sim Innovations
...
function display_lights_test(test_state)
	...
	gbl_test_state = test_state
	if test_state == 0 then
		if gbl_vvi_dial_show == 0 then
			visible(txt_leds_vvi,false)
		end
		txt_set(txt_leds_obs1,gbl_mcp_capt_crs)
		txt_set(txt_leds_warn_spd,str_speed_status)
		...
		txt_set(txt_leds_vvi,gbl_mcp_vvi)
		txt_set(txt_leds_obs2,gbl_mcp_fo_crs)
	elseif test_state == 1 then
		if gbl_power == 1 then
			visible(txt_leds_vvi,true)
			txt_set(txt_leds_obs1,"888")
			txt_set(txt_leds_warn_spd,"")
			txt_set(txt_leds_mch,"")
			txt_set(txt_leds_spd,".88888")
			txt_set(txt_leds_hdg,"888")
			txt_set(txt_leds_alt,"88888")
			txt_set(txt_leds_vvi,"+8888")
...
function set_panel_states(status,is_overspd,is_underspd)
	...
	visible(txt_leds_warn_spd,(is_overspd == 1 or is_underspd == 1) and gbl_speed_leds_show ~= 0 and gbl_power)

	visible(img_AT_led,				pos_sw_AT		== 1 and gbl_power == 1)
...
switch_cwsa				= switch_add("empty.png" ,nil,mcp_C8,mcp_L5, 70, 70,press_cwsa)
switch_cwsb				= switch_add("empty.png" ,nil,mcp_C9,mcp_L5, 70, 70,press_cwsb)
...
xpl_dataref_subscribe("laminar/B738/autopilot/n1_status"						,"INT",			n1_chg)
xpl_dataref_subscribe("laminar/B738/autopilot/speed_status1"					,"INT",			mcpspd_chg)
xpl_dataref_subscribe("laminar/B738/autopilot/vnav_status1"					,"INT",			vnav_chg)
xpl_dataref_subscribe("laminar/B738/autopilot/lvl_chg_status"					,"INT",			lvlchg_chg)
xpl_dataref_subscribe("laminar/B738/autopilot/hdg_sel_status"					,"INT",			hdgsel_chg)
xpl_dataref_subscribe("laminar/B738/autopilot/lnav_status"					,"INT",			lnav_chg)
xpl_dataref_subscribe("laminar/B738/autopilot/vorloc_status"					,"INT",			vorloc_chg)
xpl_dataref_subscribe("laminar/B738/autopilot/app_status"						,"INT",			app_chg)
xpl_dataref_subscribe("laminar/B738/autopilot/alt_hld_status"					,"INT",			alt_hold_chg)
xpl_dataref_subscribe("laminar/B738/autopilot/vs_status"						,"INT",			vs_chg)
xpl_dataref_subscribe("laminar/B738/autopilot/cmd_a_status"					,"INT",			cmda_chg)
xpl_dataref_subscribe("laminar/B738/autopilot/cmd_b_status"					,"INT",			cmdb_chg)
xpl_dataref_subscribe("laminar/B738/autopilot/cws_a_status"					,"INT",			cwsa_chg)
xpl_dataref_subscribe("laminar/B738/autopilot/cws_b_status"					,"INT",			cwsb_chg)
xpl_dataref_subscribe("laminar/B738/autopilot/master_capt_status"				,"INT",			FA_MA_A_chg)
xpl_dataref_subscribe("laminar/B738/autopilot/master_fo_status"				,"INT",			FA_MA_B_chg)
xpl_dataref_subscribe("laminar/B738/autopilot/flight_director_pos"			,"INT",			fda_switch_chg)
xpl_dataref_subscribe("laminar/B738/autopilot/flight_director_fo_pos"			,"INT",			fdb_switch_chg)
xpl_dataref_subscribe("laminar/B738/autopilot/autothrottle_status"			,"INT",			img_AT_led_chg)
...
xpl_dataref_subscribe("laminar/B738/autopilot/bank_angle_pos"					,"INT",			bank_limit_chg)
...
xpl_dataref_subscribe("laminar/B738/autopilot/disconnect_pos"					,"INT",			APengage_chg)

xpl_dataref_subscribe("sim/cockpit2/electrical/bus_volts"						,"FLOAT[6]",
					  "sim/cockpit/electrical/avionics_on"						,"INT",
					  "laminar/B738/autopilot/vvi_dial_show"					,"INT",
					  "laminar/B738/autopilot/show_ias"							,"INT",			set_power_xpl)
...
xpl_dataref_subscribe("laminar/B738/autopilot/blink"							,"INT",
						"laminar/B738/mcp/over_spd"								,"INT",
						"laminar/B738/mcp/digit_A"								,"INT",
						set_panel_states)
xpl_dataref_subscribe("laminar/B738/dspl_light_test"							,"INT",			display_lights_test)
