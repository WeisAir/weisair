...
function callback_inner_knob_turn(direction)
	...
	if direction == 1 then 
		...
		if xpdr_selected_digit == -1 then xpl_command("WeisAIR/dc3/xpdr/xpdr_mode_inc") end
	...
	if direction == -1 then
		...
		if xpdr_selected_digit == -1 then xpl_command("WeisAIR/dc3/xpdr/xpdr_mode_dec") end
...
function xpdr_values (xpdr_code, xpdr_mode,xpdr_ident)
	...
	if xpdr_mode == 0 then txt_set(txt_xpdr_mode, "OFF") end
	if xpdr_mode == 1 then txt_set(txt_xpdr_mode, "SBY") end
	if xpdr_mode == 2 then txt_set(txt_xpdr_mode, "ON") end
	if xpdr_mode == 3 then txt_set(txt_xpdr_mode, "ALT") end 
...
"les/dc3/radios/xpndr_kb","INT",	-- XPDR (mode)
