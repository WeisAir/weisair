...
local announciator_dataref_name = user_prop_add_string("Simulator Source Data Name", "laminar/B738/annunciator/at_fms_disconnect1", "The simulator data field to read annunciator status from.")
...
if user_prop_get(add_lable) == true then
	if user_prop_get(lable_pos) == "top" then 
		...
		annunciator_active = img_add("ann_active.png",0,20,45,45)
	...
	if user_prop_get(lable_pos) == "bottom" then 
		...
		annunciator_active = img_add("ann_active.png",0,0,45,45)
	...
	if user_prop_get(lable_pos) == "left" then 
		...
		annunciator_active = img_add("ann_active.png",45,0,45,45)
		txt_add(user_prop_get(lable_text), lable_txt_style, 0,0,45,45)
	end
	
	if user_prop_get(lable_pos) == "right" then 
		annunciator_inactive = img_add("ann_inactive.png",0,0,45,45)
		annunciator_active = img_add("ann_active.pngive_yellow.png",0,0,45,45)
...
function press_button()                                    
    xpl_command ( "WeisAIR/b733/mcp/fmc_prst_push") 
end
function release_button()                                 
    xpl_command ( "WeisAIR/b733/mcp/fmc_prst_release") 
end  
