...
"""run command: python3 generate_streamdeck_html.py B737-800X/sd32 \
  --icons-dir icons --icons-dir extra-icons \
  --output streamdeck.html"""

"""update 1: respects file-names property for icon lookup"""
"""update 2: allows multiple --icons-dir arguments, searched in order"""
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
      ...
      action="append",
      type=Path,
      help="Directory containing icon PNGs; may be supplied multiple times.",
    )
...
def icon_data_uri(icon_name: str | None, icons_dirs: list[Path]) -> str | None:
    if not icon_name:
        return None
...
def action_icon_name(action: dict[str, Any]) -> str | None:
    file_names = action.get("file-names")
    if isinstance(file_names, list) and file_names and file_names[0]:
        return str(file_names[0])
    return action.get("icon")


def embed_icons(tree: dict[str, list[dict[str, Any]]], icons_dirs: list[Path]) -> None:
    for actions in tree.values():
        for action in actions:
            if action:
                action["_icon_data"] = icon_data_uri(action_icon_name(action), icons_dirs)
...
def generate_html(config_dir: Path, output: Path, icons_dirs: list[Path]) -> None:
    tree = collect_tree(config_dir)
    embed_icons(tree, icons_dirs)
...
def main() -> None:
    ...
    icons_dirs = [path.resolve() for path in (arguments.icons_dirs or [])]
    generate_html(config_dir, output.resolve(), icons_dirs)
